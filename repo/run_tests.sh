...
docker compose --profile test build --quiet 2>&1
...
# Wait for app
for i in $(seq 1 30); do
  if docker compose exec -T app wget -q -O /dev/null http://localhost:8080/health 2>/dev/null; then
...
UNIT_OUTPUT=$(docker compose --profile test run --rm test go test -v -count=1 ./tests/unit/... 2>&1) || true
echo "$UNIT_OUTPUT"
...
API_OUTPUT=$(docker compose --profile test run --rm \
  -e APP_HOST=app \
  -e APP_PORT=8080 \
